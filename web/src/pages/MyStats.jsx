...
// Временная заглушка - в реальном приложении здесь будет получение user_id из авторизации
const CURRENT_USER_ID = 1 // Заменить на реальный механизм авторизации

export default function MyStats() {
  const { data: stats, isLoading } = useQuery({
    queryKey: ['stats', CURRENT_USER_ID],
    queryFn: () => api.getUserStats(CURRENT_USER_ID).then((res) => res.data),
  })

  const { data: reports } = useQuery({
    queryKey: ['reports', CURRENT_USER_ID],
    queryFn: () => api.getReports(CURRENT_USER_ID).then((res) => res.data),
    enabled: !!stats,
  })
